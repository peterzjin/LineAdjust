...
/* Character Size */
#define HAL_UART_8_BITS_PER_CHAR    8//0x00
#define HAL_UART_9_BITS_PER_CHAR    9//0x01
...
extern uint8 HalUART0pen(char*  portnr,				// portnumber (1..4)
...
extern uint8 CommThread( halUARTCfg_t *config );
 ...
 */
extern uint8 HalUARTRead ( uint8 port, uint8 *pBuffer, uint16 length );

extern uint8 ReadChar ( uint8 port, uint8 *pBuffer, uint16 length );
/*
 * Write a buff to the uart *
 */
extern uint8 HalUARTWrite ( uint8 port, uint8 *pBuffer, uint16 length );
 ...
 */
extern void Hal_UART_FlowControlSet ( uint8 port, boolean status );
